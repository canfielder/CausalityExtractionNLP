...
## Tabulizer
Since **pdftools** did not perform well enough, we will try to package **Tabulizer**, which also requires **RJava** to be installed.
...
#### Split New Lines
The data comes with an Excel spreadsheet filled with specific strings known in the available data that are to be dropped. The pattens have already been imported. Next, any element in the vecotr which contains a newline, is split into two (or more), separate vectors. 
```{r}
